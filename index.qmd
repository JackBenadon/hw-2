...
abalone <- read_csv(url) # Insert your code here
...
plot <- ggplot(dataframe, aes(x = values)) +
  geom_histogram() + 
...
ggplot(df) +
  ...
  theme_minimal() # Insert your code for boxplot here
```
...
ggplot(df) +
  ...
  theme_minimal() # Insert your code for violinplot here
```
The abalones with `sex = 'F'` tend to be very slightly larger on average than those that have `sex = 'M'` in terms of both diameter and length. However, they are both significantly larger than those with `sex = 'I'`.
...
ggplot(df) +
  ...
  theme_minimal() # Insert your code here
```
There is one notable anomaly in the dataset there is a abalone that has a far larger diameter than the others from its sex and it is not as long as you would expect an abalone to be with a diameter that size.
...
```{R}
ggplot(df) +
  aes(x = length, y = diameter, color = sex) +
...
ggplot(filter_data, aes(x= mean, y = variables)) +
...
```{R}
df_numeric <- df %>% select_if(is.numeric) # Select only the numeric variables
cor_table <- round(cor(df_numeric), 2)
cor_table
```
...
```{R}
quant_vars <- c("length", "diameter", "height", "whole_weight", 
                "shucked_weight", "viscera_weight", "shell_weight", "rings")

scatter_plots <- 
  map2(quant_vars, quant_vars, ~ ggplot(df, aes_string(x = "rings", y = .x, color = "sex")) + geom_point() + labs(x = "Number of Rings", y = .x))
...
model
```
This means that for every one unit change in diameter we expect height to change by 1.9783.
...
abline(lm(df$height ~ df$diameter), col = "red")
```
Yes it is an apropriate fit because the data is linear. (except for some outliers)
...
new_df <- data.frame(new_diameters)
  height <- predict(model, new_df = new_df)
